// src/profiles/FootballerProfile.jsx
import React from "react";

export default function FootballerProfile({ user }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 to-black text-white p-6">
      <div className="max-w-3xl mx-auto bg-[#111] p-6 rounded-2xl shadow-lg">
        <div className="text-center">
          <img
            src={user.avatar}
            className="w-28 h-28 rounded-full mx-auto border-4 border-green-500 shadow"
            alt="avatar"
          />
          <h1 className="text-3xl font-bold mt-4">{user.name}</h1>
          <p className="text-gray-400">@{user.username} • Yosh: {user.age}</p>
          <p className="text-green-400 mt-2 font-semibold">⚽ Futbolchi</p>
        </div>

        <div className="mt-6">
          <h2 className="text-xl font-semibold">🏆 Futbol Level</h2>
